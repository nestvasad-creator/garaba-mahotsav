-- ==============================================================================
-- EVENT ID CARD MANAGEMENT SYSTEM — COMPLETE PRODUCTION SETUP SCRIPT
-- ==============================================================================
-- Features Included:
-- 1. Full relational schema with physical_form_number (Mandatory) & unique constraint
-- 2. Clean English card categories (no Gujarati encoding artifacts)
-- 3. Dynamic category & gender theme palette engine
-- 4. Complete RBAC permissions catalog & role mappings (all 7 roles)
-- 5. Storage buckets & RLS access control policies
-- 6. ZERO dummy data (zero fake participants, cards, or logs)
-- 7. Test Auth accounts for each of the 7 user roles with test names
--
-- Instructions:
-- 1. Open your Supabase Dashboard (https://supabase.com/dashboard)
-- 2. Go to: SQL Editor (left navigation)
-- 3. Click: "New query"
...
-- ------------------------------------------------------------------------------
-- 2. CLEAN TEARDOWN (DELETE FIRST IF EXISTS TO PREVENT CONFLICTS)
-- ------------------------------------------------------------------------------
...
-- Ensure physical_form_number is present and constrained on existing databases
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'registrations' AND column_name = 'physical_form_number'
  ) THEN
    ALTER TABLE registrations ADD COLUMN physical_form_number VARCHAR(100);
    UPDATE registrations SET physical_form_number = 'LEGACY-' || registration_number WHERE physical_form_number IS NULL;
    ALTER TABLE registrations ALTER COLUMN physical_form_number SET NOT NULL;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'uq_registrations_event_physical_form'
  ) THEN
    ALTER TABLE registrations ADD CONSTRAINT uq_registrations_event_physical_form UNIQUE (event_id, physical_form_number);
  END IF;
END $$;
...
-- ------------------------------------------------------------------------------
CREATE INDEX IF NOT EXISTS idx_registrations_mobile ON registrations(event_id, mobile);
CREATE INDEX IF NOT EXISTS idx_registrations_status ON registrations(event_id, status);
CREATE INDEX IF NOT EXISTS idx_registrations_physical_form_no ON registrations(event_id, physical_form_number);
CREATE INDEX IF NOT EXISTS idx_documents_reg ON documents(registration_id);
CREATE INDEX IF NOT EXISTS idx_id_cards_qr ON id_cards(event_id, qr_code_hash);
CREATE INDEX IF NOT EXISTS idx_print_jobs_status ON print_jobs(event_id, status);
CREATE INDEX IF NOT EXISTS idx_qr_scans_card ON qr_scans(card_id, scanned_at DESC);

-- ------------------------------------------------------------------------------
-- 5. PERMISSIONS CATALOG & RBAC MATRIX
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 9. USER AUTH SEED (ALL 7 USER ROLES WITH TEST NAMES ONLY)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- CLEANUP COMPLETE: ZERO DUMMY DATA
-- ------------------------------------------------------------------------------
-- All participant tables (registrations, documents, id_cards, print_jobs, qr_scans)
-- are completely clean and ready for live production enrollment.
-- ==============================================================================
